...
export function flattenAction(sourceFile: string, targetFile: string, options: FlattenOptions): Promise<string> {
    ...
    return validateSourceTarget({name: sourceFile, title: 'source file'}, {name: targetFile, title: 'target file'})
            .then(valid => { if (valid === validated())       {
                                                               return sourceOpTarget<JsonLd>(sourceFile, targetFile, jsonld.flatten, options);
                                                              }
                                else { return valid; }
                            });
}
